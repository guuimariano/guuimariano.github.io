...
export function renderPenaltiesList(penaltyTypes, handlers) {
    const frag = document.createDocumentFragment();
    const list = document.createElement('ul');
    list.style.listStyle = 'none';
    list.style.padding = '0';
    (Array.isArray(penaltyTypes) ? penaltyTypes : []).forEach((pt) => {
        const li = document.createElement('li');
        ...
        subtitle.textContent = `${pt.severity || 'MINOR'}${pt.description ? ' · ' + pt.description : ''}`;
        ...
        be.className = 'edit-btn';
        be.textContent = 'Editar';
        be.addEventListener('click', () => handlers?.onEdit && handlers.onEdit(pt));
        const bd = document.createElement('button');
        bd.className = 'delete-btn';
        bd.textContent = 'Excluir';
        bd.addEventListener('click', () => handlers?.onDelete && handlers.onDelete(pt));
        actions.appendChild(be);
        actions.appendChild(bd);
        li.appendChild(left);
        li.appendChild(actions);
